...
|---:|---|---:|---:|---:|---|---|---|---|---|---|---|
| 1 | GOOD | 949 | 155.568 | 0.969 | NA | — | A1 | NA | NA | C | act 层待定：good+better 归并，comparative 多嵌于目标态框架（make it better=A3）vs 评价（A1）；LL 全组最高，须查〔concordance 判定：act=A1〕 |
| 2 | language | 642 | 150.991 | 1.210 | G2 | Wording | NA | NA | PENDING | C | 维度三待定：your language background/first language(C1) vs academic language(G2 Wording)；本组理论枢纽词〔concordance 判定：d1=G2-Wording〕 |
...
| 5 | academic | 841 | 73.006 | 0.669 | G2 | Wording | NA | NA | NA | C | 维度一待定：属 register，手册列 tone/formal/register 为 PENDING〔concordance 判定：d1=G2-Wording〕 |
| 6 | STRENGTH | 257 | 62.497 | 1.237 | NA | — | A1 | NA | NA | C | act 层待定：反馈自身小标题"Strengths:"（元话语 NA）vs 归功（A1）；且属 strong 词族，按 v3 强制查询规则不得凭词形定〔concordance 判定：act=A1〕 |
...
| 10 | USE | 1068 | 40.969 | 0.427 | NA | — | NA | NA | NA | C | 维度一待定：your use of evidence(G1)/word use(G2)/use of tenses(G2)；act 层待定：use more examples(A3)vs 描述(NA)〔concordance 判定：d1=NA，act=NA〕 |
| 11 | ARTICLE | 167 | 39.363 | 1.213 | G2 | Grammar | NA | NA | NA | C | 维度一待定：英语冠词(G2-Grammar)vs 引用的文章(G1-Development)；L2 语法标记的关键判别点〔concordance 判定：d1=G2-Correctness-Grammar〕 |
...
| 16 | suggestions | 160 | 34.698 | 1.151 | NA | — | A3 | NA | NA | C | act 层待定：反馈小标题"Suggestions:"（元话语 NA）vs 名词化建议行为（A3）〔concordance 判定：act=A3(100%)〕 |
...
| 22 | important | 323 | 25.774 | 0.639 | NA | — | NA | NA | NA | C | act 层待定：an important point(A1)vs it is important to…(A3 框架)〔concordance 判定：act=NA〕 |
...
| 25 | understandable | 62 | 22.275 | 1.600 | G1 | Ideas | A1 | NA | NA | C | 维度一待定（clear 系）；act 低信度：作为"低门槛褒扬"（your English is understandable）与 A1 定义是否相符须查〔concordance 判定：d1=G1-Ideas〕 |
| 26 | too | 529 | 22.252 | 0.449 | NA | — | A2 | NA | NA | M | 标记过度=内在负向（同组 1 overly 口径）；polysemy 高于 overly，信度中等 |
| 27 | advice | 128 | 21.853 | 0.994 | NA | — | A3 | NA | NA | C | act 层待定：反馈小标题（NA）vs my advice is to…（A3）〔concordance 判定：act=A3〕 |
| 28 | expressions | 49 | 21.536 | 1.846 | G2 | Wording | NA | NA | NA | H | Wording 明示 |
| 29 | CLEAR | 1289 | 21.466 | 0.274 | G1 | Ideas | NA | NA | NA | C | 维度一待定（手册 clear 系）；act 层待定：clear(A1)vs clearer 目标态框架(A3)〔concordance 判定：d1=G1-Ideas，act=NA〕 |
...
| 41 | strong | 711 | 16.819 | 0.330 | NA | — | A1 | NA | NA | C | 手册 v3 强制查询清单（A1/A3 争议），不得凭词形归类〔concordance 判定：act=A1(91%)〕 |
| 42 | may | 404 | 15.676 | 0.430 | NA | — | NA | M1 | NA | H | 手册 v3 M1 明示 hedge |
| 43 | final | 414 | 15.633 | 0.423 | NA | — | NA | NA | NA | C | 维度一待定：your final paragraph(G1-Structure)vs"Final thoughts:"小标题(NA)〔concordance 判定：d1=NA〕 |
| 44 | topic | 602 | 15.441 | 0.344 | G1 | Ideas | NA | NA | NA | C | 维度一待定：the topic of your essay(G1-Ideas)vs topic sentence(G2/G1 边界)〔concordance 判定：d1=G1-Ideas〕 |
...
| 49 | informal | 190 | 12.535 | 0.575 | G2 | Wording | A2 | NA | NA | C | 维度一待定（register）；学术语境下标记偏离=负向（同组 1 casual 口径）〔concordance 判定：d1=G2-Wording〕 |
...
| 56 | correct | 152 | 10.825 | 0.600 | G2 | Correctness | A3 | NA | NA | C | act 层待定：correct these errors(A3)vs the correct form(NA)〔concordance 判定：act=A3〕 |
...
| 61 | patterns | 56 | 10.274 | 1.038 | G2 | Grammar | NA | NA | NA | C | 维度一待定：error patterns(G2)vs sentence patterns(G2-Local)vs 论证模式(G1)〔concordance 判定：d1=G2-Correctness-Grammar(38%)〕 |
| 62 | briefly | 119 | 10.223 | 0.666 | NA | — | NA | M1 | NA | C | hedge 层待定：briefly explain 削减要求强度（M1）vs 单纯方式副词（NA）〔concordance 判定：hedge=M1(71%)〕 |
...
| 66 | consistency | 105 | 8.990 | 0.665 | G2 | Grammar | NA | NA | NA | C | 维度一待定：consistency of tense(G2)vs of argument(G1)〔concordance 判定：d1=G2-Correctness-Grammar〕 |
| 67 | background | 105 | 8.990 | 0.665 | G1 | Development | NA | NA | NA | C | 维度三待定：your language background(C1)vs background information(G1-Development)〔concordance 判定：d1=G1-Development，c=NA〕 |
| 68 | can | 680 | 8.843 | 0.241 | NA | — | NA | NA | NA | C | 与 could/might/would 同类共享项：you can add(A3)vs this can be confusing(M1)；两层分别判〔concordance 判定：act=NA〔裸 NA 推定覆盖全部剩余 PENDING 层〕，hedge=NA〔裸 NA 推定覆盖全部剩余 PENDING 层〕〕 |
| 69 | check | 144 | 8.774 | 0.550 | G2 | Mechanics | A3 | NA | NA | C | 维度一待定：check your spelling(G2)vs check your logic(G1)；手册 A3 明示〔concordance 判定：d1=G2-Correctness-Mechanics〕 |
...
| 72 | mention | 130 | 8.417 | 0.569 | NA | — | NA | NA | NA | C | act 层待定：you mention X（描述 NA）vs consider mentioning（A3）〔concordance 判定：act=NA〕 |
...
| 76 | complex | 76 | 8.273 | 0.763 | NA | — | NA | NA | NA | C | 维度一待定：complex sentences(G2-Local)vs complex ideas(G1)〔concordance 判定：d1=NA〕 |
| 77 | suggested | 86 | 8.255 | 0.710 | NA | — | A3 | NA | NA | C | act 层待定：the suggested revision（NA）vs I suggested（A3）〔concordance 判定：act=A3〕 |
...
| 82 | write | 264 | 7.688 | 0.368 | NA | — | A3 | NA | NA | C | act 层待定：when you write（描述 NA）vs write this as…（A3）〔concordance 判定：act=A3〕 |
...
| 86 | only | 333 | 7.195 | 0.314 | NA | — | NA | NA | NA | C | hedge 层待定：最小化降调（only a few errors=M1）vs 限定（NA）；同组 1 just〔concordance 判定：hedge=NA〕 |
| 87 | complete | 99 | 7.088 | 0.602 | NA | — | NA | NA | NA | C | 维度一待定：complete sentence（句子残缺，G2-Grammar）vs complete your argument(G1)〔concordance 判定：d1=NA〕 |
...
| 95 | natural | 49 | 5.731 | 0.795 | NA | — | NA | NA | NA | C | 三层皆待定：sounds natural 是否以母语者语感为隐含标准（C1）；若判 C1 则维度一二依规则归 NA〔concordance 判定：d1=NA，act=NA，c=NA〕 |
...
| 109 | focus | 466 | 4.919 | 0.216 | G1 | Ideas | NA | NA | NA | C | 维度一待定：your focus(G1)vs focus on this sentence(G2)；act 层待定：focus on X(A3)vs 描述(NA)〔concordance 判定：d1=G1-Ideas，act=NA〕 |
...
| 116 | accurate | 63 | 4.346 | 0.589 | G1 | Ideas | A1 | NA | NA | C | 维度一待定：accurate facts(G1-Dev)vs accurate grammar(G2)；正向评价〔concordance 判定：d1=G1-Ideas〕 |
| 117 | her | 91 | 4.326 | 0.480 | NA | — | NA | NA | NA | M | 疑似议题内容残留，待核 |
| 118 | could | 499 | 4.275 | 0.194 | NA | — | A3 | M1 | NA | C | 手册指定共享项，抽 50 行分别判 act 层与 hedge 层〔concordance 判定：act=A3(53%)，hedge=M1(53%)〕 |
...
| 121 | reduce | 139 | 4.200 | 0.376 | NA | — | A3 | NA | NA | C | 维度一待定：reduce wordiness(G2)vs reduce scope(G1)〔concordance 判定：d1=NA〕 |
...
| 126 | unclear | 153 | 4.114 | 0.353 | G2 | Wording | A2 | NA | NA | C | 维度一待定（手册 clear 系）；负向评价〔concordance 判定：d1=G2-Wording(29%)〕 |
| 127 | emotional | 217 | 4.076 | 0.292 | NA | — | NA | NA | NA | C | 维度一待定：emotional language(G2-Wording/register)vs emotional appeal(G1-Ideas)；亦可能为议题残留〔concordance 判定：d1=NA〕 |
| 128 | especially | 204 | 4.066 | 0.301 | NA | — | NA | NA | NA | M | 焦点副词 |
| 129 | neutral | 62 | 3.980 | 0.566 | NA | — | NA | NA | NA | C | 维度一待定（tone/register）；客观性规范框架词〔concordance 判定：d1=NA〕 |
| 130 | discuss | 110 | 3.913 | 0.410 | G1 | Ideas | NA | NA | NA | C | act 层待定：you discuss X（描述 NA）vs discuss counterarguments（A3）〔concordance 判定：act=NA〕 |
...
|---|---:|---:|---:|---:|---:|
| G2 | 27 | 54.0% | 6604 | 46.2% | 0.914 |
| G1 | 23 | 46.0% | 7688 | 53.8% | 0.516 |
| **已定标签合计** | **50** | **100.0%** | **14292** | **100.0%** | — |
| N/A（不计入分母） | 78 | — | — | — | — |
| PENDING（不计入分母） | 3 | — | — | — | — |
| 清单总数 | 131 | — | — | — | — |
...
| 主类 | 子类 | 词位数 | 占比 |
|---|---|---:|---:|
| G1 | Development | 5 | 10.0% |
| G1 | Global Structure | 4 | 8.0% |
| G1 | Ideas | 14 | 28.0% |
| G2 | Correctness | 4 | 8.0% |
| G2 | Grammar | 9 | 18.0% |
| G2 | Local Structure | 3 | 6.0% |
| G2 | Mechanics | 2 | 4.0% |
| G2 | Wording | 9 | 18.0% |
...
| 标签 | 词位数 | 占已定标签 | Freq_Tar 合计 | 占已定标签词次 | LR 均值 |
|---|---:|---:|---:|---:|---:|
| A3 | 15 | 51.7% | 3147 | 47.8% | 0.571 |
| A1 | 10 | 34.5% | 2471 | 37.5% | 0.855 |
| A2 | 4 | 13.8% | 966 | 14.7% | 0.460 |
| **已定标签合计** | **29** | **100.0%** | **6584** | **100.0%** | — |
| N/A（不计入分母） | 102 | — | — | — | — |
| PENDING（不计入分母） | 0 | — | — | — | — |
| 清单总数 | 131 | — | — | — | — |
...
| 标签 | 词位数 | 占已定标签 | Freq_Tar 合计 | 占已定标签词次 | LR 均值 |
|---|---:|---:|---:|---:|---:|
| M1 | 7 | 100.0% | 2226 | 100.0% | 0.517 |
| **已定标签合计** | **7** | **100.0%** | **2226** | **100.0%** | — |
| N/A（不计入分母） | 123 | — | — | — | — |
| PENDING（不计入分母） | 1 | — | — | — | — |
| 清单总数 | 131 | — | — | — | — |
...
|---|---:|---:|---:|
| 现状（PENDING 不计入） | 23 | 27 | 46.0% |
| 3 个 PENDING 全归 G1（上界） | 26 | 27 | 49.1% |
| 3 个 PENDING 全归 G2（下界） | 23 | 30 | 43.4% |

### B9 concordance 待办清单

共 **7** 个词族需 concordance 判定（含 2 个低信度已定项）。

| Type | Freq_Tar | 待定层 | 竞争读法 |
|---|---:|---|---|
| `language` | 642 | 维度三 | 维度三待定：your language background/first language(C1) vs academic language(G2 Wording)；本组理论枢纽词〔concordance 判定：d1=G2-Wording〕 |
...
| understandable | 22.275 | 1.600 | G1/Ideas | A1 | NA |
